...
const confirmLevelLabels: Record<string, string> = {
  auto: "全部自动确认",
  low: "仅破坏性操作确认",
  medium: "文件修改确认",
  high: "全部需确认",
...
export function AgentInfoSection() {
  ...
  const [editValue, setEditValue] = useState(settings.authorName);

  const handleSave = () => {
    if (editValue.trim()) {
      updateSettings({ authorName: editValue.trim() });
    }
    setEditing(false);
  };
    ...
    <SidebarSection title="Agent 信息">
      <div className="flex flex-col gap-[10px]">
        ...
        <div className="flex items-center justify-between">
          ...
          {editing ? (
            ...
            <span
              ...
              onClick={() => { setEditValue(settings.authorName); setEditing(true); }}
            >
              {settings.authorName || "未设置"}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-[13px] font-medium text-text-primary">
            {confirmLevelLabels[settings.confirmLevel] ?? settings.confirmLevel}
          </span>
        </div>
      </div>
    </SidebarSection>
