...
- **高度可配**：用户自主配置 LLM 服务、自定义 Skills、Prompt 模板等
...
4. **渐进增强**：核心功能开箱即用，高级功能（自定义Skill、模板）按需使用
...
- 支持按文件类型过滤搜索范围
- 搜索结果高亮显示匹配内容
- Agent可通过Skill调用搜索功能，基于搜索结果进行文档操作
...
- **Tool调用节点**：显示调用的Skill名称、参数、执行状态
- **执行结果节点**：显示Skill执行的结果摘要
- **最终回复节点**：Agent的最终输出
...
- 用户可编写自定义Skill脚本（支持TypeScript/Python）
- 自定义Skill通过标准接口与Agent交互
- Skill声明文件定义：名称、描述（供LLM理解）、参数Schema、执行入口
- Skill脚本存储在应用数据目录（非用户工作区）
- 用户可在设置中管理（添加、编辑、删除、启用/禁用）自定义Skill
...
```json
{
  ...
  "entry": "skills/generate_changelog.py"
}
```
...
- 会话数据包含：消息记录、Tool调用记录、Token消耗、关联工作区

#### 3.7.2 历史会话列表

- 侧边栏或独立面板展示历史会话列表
- 每个会话显示：标题（自动生成或用户编辑）、时间、Token消耗、关联工作区
- 支持搜索历史会话
- 支持删除历史会话
...
- 可折叠/展开
- 各区域（文件树、Agent信息、Todo列表、Token统计）可独立折叠
- 文件树区域支持搜索过滤
...
- 内置Skills列表（启用/禁用）
- 自定义Skills管理（添加、编辑、删除）
- Skill编辑器（代码编辑 + 声明文件编辑）
...
| total_input_tokens | INTEGER | 总输入Token数 |
| total_output_tokens | INTEGER | 总输出Token数 |
...
- 存储位置：`{应用数据目录}/skills/custom/`
- 每个Skill一个目录，包含：
  - `skill.json`：Skill声明文件
...
- 内置Skill：generate_document / modify_document / read_document
- Word/Markdown文档支持
- 基础UI布局（主界面区 + 右侧栏）
- 会话持久化
- 版本快照
...
| Skill | Agent可调用的工具，封装了特定的文档操作能力 |
