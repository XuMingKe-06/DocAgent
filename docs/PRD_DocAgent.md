...
| 代码执行 | RestrictedPython + 自定义沙箱 | Code Interpreter 安全执行 |
| 图表绘制 | matplotlib | 嵌入文档的图表生成 |
...
|  |  - 5个Handler（read/convert/analyze）       |  |
|  |  - 8个Tool（文件系统操作）                   |  |
|  |  - SQLite数据库管理（6张表）                  |  |
|  |  - 版本快照管理                              |  |
|  +--------------------------------------------+  |
...
用户输入 → AgentExecutor → LLM API（流式响应）
              ...
              │               │               │
         Handler          Code           Tool
         (read/          Interpreter    (list/search/
          convert/        (generate/     read/write/
          analyze)        modify)        delete/...)
              │               │               │
              ▼               ▼               ▼
        Python Sidecar    Python Sidecar   Rust 原生
              │               │               │
              └───────┬───────┘               │
                      ▼                       ▼
              文件系统操作（工作区目录）
...
5. Agent 执行对应 Handler/Tool/Code Interpreter
6. 高风险操作需用户确认
7. 结果回注给 LLM，继续推理
...
| code_interpreter_handler | 执行 Python 代码（生成/修改文档） |
...
|------|------|---------|---------|-----------|
| Word (.docx) | Handler | Handler | Handler | Code Interpreter |
| Excel (.xlsx) | Handler | Handler | Handler | Code Interpreter |
| PPT (.pptx) | Handler | Handler | Handler | Code Interpreter |
| PDF (.pdf) | Handler | Handler | Handler | Code Interpreter |
| Markdown (.md) | Handler | Handler | Handler | Tool write_text_file |

生成和修改通过 Code Interpreter 执行 Python 代码实现，使用预设的 Helper 函数库。
...
### 3.9 Code Interpreter 安全沙箱

- 模块白名单/黑名单
- 正则安全检查（os.system/__import__/eval/exec 等）
- 文件路径隔离（只允许写入工作区目录）
- 子进程隔离（崩溃不影响主进程）
- 执行超时（60 秒）
- 内存限制（512MB）
...
|   │ [思考] 分析需求...              │  |  │  ├── report.docx   │  |
|   │ [内容] 我来帮你生成...           │  |  │  ├── data.xlsx     │  |
|   │ [工具] 调用 Code Interpreter    │  |  │  └── slides.pptx   │  |
|   │ [结果] 文档已生成               │  |  └────────────────────┘  |
...
- **Sidebar**: 文件树 + Agent信息 + Token统计 + Todo
- **PreviewOverlay**: 文档预览浮层（懒加载）
- **SettingsDialog**: 8个标签页设置弹窗（懒加载）
- **HistoryPanel**: 历史会话面板（懒加载）
