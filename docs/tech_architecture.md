...
│   │   │   │   └── builtin.rs       # 5个文档类型内置Handler注册
│   │   │   ├── tool/                # Tool执行引擎（始终启用）
│   │   │   │   ├── mod.rs
│   │   │   │   ├── trait_def.rs     # Tool trait定义
│   │   │   │   ├── registry.rs      # Tool注册表
│   │   │   │   └── builtin.rs       # 8个文件系统内置Tool注册
...
│   │   ├── code_handler.py          # 代码解释器Handler (RestrictedPython)
│   │   ├── validator.py             # 文档校验器
│   │   ├── font_utils.py            # 字体工具
│   │   └── doc_helpers/             # 文档生成Helper函数库
│   │       ├── word_helpers.py / excel_helpers.py / ppt_helpers.py
│   │       ├── pdf_helpers.py / chart_helpers.py / common.py
│   └── tests/
│       └── test_code_security.py    # 代码安全测试
├── docs/                            # 开发文档
├── shared/                          # 前后端共享类型
│   └── types.ts
├── package.json
...
| **Handler Engine** | 文档处理Handler注册和执行，通过Python Sidecar实现 | `HandlerRegistry` (5个内置文档Handler) |
| **Tool Engine** | 文件系统基础操作，始终启用，纯Rust实现 | `ToolRegistry` (8个内置Tool) |
...
AgentExecutor
  ...
  │     System Prompts: document_design, code_interpreter, task_type, token_budget
  │
  ▼ (循环)
LLM Adapter → HTTP请求 → LLM API (流式)
  ...
  ├─→ agent:code_streaming (代码解释器流式代码)
  │
  ▼ (本轮结束)
  ├─→ 增量持久化消息到 SQLite
...
Handler/Tool 调用 → 根据类型分发
  ...
  │
  ├─→ 代码解释器 Handler (code)
  │    ├─ generate: 通过 Python 代码生成文档（替代旧 generate_document）
  │    └─ modify:   通过 Python 代码修改文档（替代旧 modify_document）
  │    └─ execute:  执行任意 Python 代码
  │
  └─→ 文件系统 Tool (纯 Rust，8个)
       ├─ list_directory: 列出目录
       ├─ search_files:   搜索文件
...
- 可配置环境变量 `DOCAGENT_PYTHON` 指定 Python 解释器路径
...
- 删除操作可选备份，强制工作区路径校验
- Code Interpreter 代码执行安全：禁止导入黑名单模块（socket, shutil, subprocess 等）
- Code Interpreter 文件写入隔离：`safe_open` 拒绝写入工作区外路径
- 代码执行在独立子进程中运行，崩溃不影响主 Sidecar 进程
...
- 默认超时：120秒（代码执行），60秒（文档操作）

### 8.2 Handler 与 Tool 分工

- **Handler**（5个，通过 Python Sidecar）：文档类型专业处理（word/excel/ppt/pdf/markdown），仅支持 read/convert/analyze
- **Code Interpreter Handler**（1个）：代码生成/修改文档，替代旧 generate/modify，执行 Python 代码
- **Tool**（8个，纯 Rust）：文件系统基础操作，始终启用
...
6. Python Sidecar 进程启动（路径检测：环境变量 `DOCAGENT_PYTHON` / py / python / python3）
7. DocumentService + HandlerRegistry（5个内置文档处理器）
8. ToolRegistry（8个内置文件系统工具）
9. AppState 注册
