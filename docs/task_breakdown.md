...
| 任务ID | T1.17 |
| 名称 | 主界面区布局 |
| 描述 | 实现应用主界面的整体布局框架，采用三栏布局：左侧栏（会话列表）、中间区（对话 + 时间线）、右侧栏（Agent 信息 + Todo）。包括：响应式布局、面板折叠/展开、布局状态持久化、拖拽调整面板宽度。 |
| 前置依赖 | T1.3 |
...
#### T1.20：右侧栏（Agent 信息 + Todo）

| 属性 | 内容 |
|------|------|
| 任务ID | T1.20 |
| 名称 | 右侧栏（Agent 信息 + Todo） |
| 描述 | 实现右侧栏的两个核心面板：Agent 信息面板（当前使用的模型、Provider、状态）、Todo 面板（Agent 生成的任务清单，可勾选完成）。 |
| 前置依赖 | T1.10, T1.17 |
| 涉及文件 | `src/components/AgentInfoPanel.tsx`, `src/components/TodoPanel.tsx` |
| 预估工时 | 1.5 人天 |
| 验收标准 | 1) Agent 信息面板正确显示当前模型和状态；2) Todo 面板可展示 Agent 生成的任务列表并支持勾选；3) 两个面板可折叠/展开 |
...
**缓解措施：**
1. 在 Agent 策略中优化 Prompt 长度，减少不必要的上下文
2. 提供本地模型（Ollama 等）支持作为低成本替代方案
