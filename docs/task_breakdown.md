...
| 任务ID | T3.3 |
| 名称 | Token 统计 |
| 描述 | 实现 Token 用量的统计分析功能。包括：单次会话 Token 统计、日/周/月用量统计、费用估算（基于各 Provider 定价）、用量趋势图表、导出用量报告。 |
| 前置依赖 | T1.20, T1.4 |
| 涉及文件 | `src-tauri/src/db/token_repo.rs`, `src-tauri/src/services/token_service.rs`, `src/components/UsageChart.tsx` |
| 预估工时 | 2 人天 |
| 验收标准 | 1) 可实时统计当前会话的 Token 用量；2) 可查看历史用量趋势；3) 费用估算基于实际定价计算 |
...
**缓解措施：**
1. 在 T3.3 阶段实现 Token 费用统计功能
2. 在 Agent 策略中优化 Prompt 长度，减少不必要的上下文
3. 提供本地模型（Ollama 等）支持作为低成本替代方案
