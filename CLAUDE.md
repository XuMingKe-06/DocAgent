...
│  ├─ services/          前端服务层: tauri.ts (invoke封装), event.ts (事件监听+Payload类型定义),
│  │                        errorHandler.ts
│  ├─ hooks/             useAgent.ts (Agent交互核心hook: sendMessage, stopAgent, confirmOperation 等)
│  └─ types/             TypeScript类型定义 (与Rust后端对齐: workflow, session, workspace, document,
│                           settings) — 需与 Rust models/ 手动同步
│
├─ src-tauri/            Rust 后端
...
│  │  │  ├─ network_monitor.rs 网络状态监控 (Online/Offline检测，触发LLM重连)
